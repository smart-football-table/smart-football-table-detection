...
client = InfluxDBClient('localhost', 8086, 'admin', 'admin', 'positions2')
client.create_database('positions2')
...
cap = cv.VideoCapture('test_logitech_1_low.avi')
...
while(True):
    ...
    if len(cnts) > 0:
        ...
        M = cv.moments(c)
        center = (int(M["m10"] / M["m00"]), int(M["m01"] / M["m00"]), datetime.datetime.now())

        json_body = [
        {
            "measurement": "ball_positons",
            "tags": {
                "host": "kicker01",
                "region": "de"
                },
